...
# # #
 N = 130 ;  d=interval(70.0) ; N0 = 90; 
 precision_param = 80 ;        setprecision(precision_param)
 μ = interval(0.32) ;  v1 = interval(-1.6) ; v2 = interval(1.0) 
...
#
# W = load("W032_70_130.jld2","W")
 
# prod_130=load("prod_130.jld2","prod_130") ; D_inv = load("D_inv.jld2","D_inv")

#  W = interval.(big.(W))
#  S = trace(N,80); C=S';
# U = W - (C.*D_inv)*solve(Matrix(prod_130),vec(S*W))
# display("U created")

# U = Sequence(fourier,vec(U))
# U2 = U*U
# jldsave("Wh_032_70_130_projected.jld2" ; U)
# jldsave("Wh_032_70_130_square.jld2" ; U2)

#
U = load("Wh_032_70_130_projected.jld2","U")
...
final_q_hexa = qq
jldsave("final_q_hexa.jld2" ; final_q_hexa)





# U = load("Wc_028_40_100.jld2","W")

# d = 40
# N1 = 60 ; μ = 1/0.28; v2 = μ ; v1 = -1.6/0.28
# U = Float64.(mid.(U)); d=mid(d); v1 = mid(v1); v2=mid(v2);μ = mid(μ)

# fourier = Fourier(N1, π/d)⊗Fourier(N1, π/d)
# U0 = Sequence(Fourier(N1, π/d)⊗Fourier(N1, π/d),zeros((2*N1+1)^2))

# for n1 = -N1:N1
#   for n2 = -N1:N1
#     U0[(n1,n2)] = U[(abs(n1),abs(n2))]
#   end
# end

# ∂1 = project(Derivative((2,0)), fourier, fourier,Float64)
# ∂2 = project(Derivative((0,2)), fourier, fourier,Float64)
# Δ = copy(∂1) ;  radd!(Δ,∂2) ; 
...
# N2 = 100
# fourierc = CosFourier(N2, π/d)⊗CosFourier(N2, π/d)
# ∂1 = project(Derivative((2,0)), fourierc, fourierc,Float64)
# ∂2 = project(Derivative((0,2)), fourierc, fourierc,Float64)
# Δ = copy(∂1) ;  radd!(Δ,∂2) ; 

# ∂1 = Nothing ; ∂2 = Nothing

# L = LinearOperator(fourierc,fourierc,Diagonal((diag(coefficients(Δ+I))).^2)) ;  Δ = Nothing
# L = I + μ*L

# V0 = 2*v1*U0 + 3*v2*(U0*U0)
# DG = project(Multiplication(V0),fourier, fourier,Float64)

# E = eigen(Hermitian(coefficients(L+DG)))

# P = E.values
# P = interval.(P).±0.00000001
# Pinv = P'
# K = Matrix(1.0*I,(2*N1+1)^2,(2*N1+1)^2)
